  ...
  db.query(sql, [username])
    .then(result => {
      argon2
        ...
        .then(isMatching => {
          if (isMatching) {
            const payload = { username: result.rows[0].username, userId: result.rows[0].userId };
            res.status(200).json({ token: jwt.sign(payload, process.env.TOKEN_SECRET), user: payload });
          } else {
            throw new ClientError(401, 'invalid login');
          }
        })
